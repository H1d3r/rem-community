...
func TestNewExternalC2OutboundRequiresDest(t *testing.T) {
	...
	_, err := NewExternalC2Outbound(map[string]string{}, &serve.Dialer{})
	if err == nil || !strings.Contains(err.Error(), "dest not found") {
		t.Fatalf("expected dest error, got %v", err)
	}
}
...
func TestHandleWritesStagerFrame(t *testing.T) {
	...
	outbound, err := NewExternalC2Outbound(map[string]string{"dest": "127.0.0.1:50050"}, &serve.Dialer{Conn: server})
	if err != nil {
		t.Fatalf("NewExternalC2Outbound returned error: %v", err)
	}

	stream := serve.NewReadWriteCloser(nil)
	conn, err := outbound.(*ExternalC2Plugin).Handle(stream, serve.NewConn(nil))
	if err != nil {
...
func TestRelayPassthrough(t *testing.T) {
	...
	conn := serve.NewConn(nil)
	got, err := inbound.(*ExternalC2Plugin).Relay(conn, serve.NewReadWriteCloser(nil))
	if err != nil {
...
func TestOutboundName(t *testing.T) {
	...
	outbound, err := NewExternalC2Outbound(map[string]string{"dest": "127.0.0.1:50050"}, &serve.Dialer{})
	if err != nil {
		t.Fatalf("NewExternalC2Outbound returned error: %v", err)
	}
