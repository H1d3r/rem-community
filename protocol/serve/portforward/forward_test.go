...
func TestHandleRequiresDialer(t *testing.T) {
	...
	_, err := plug.Handle(serve.NewReadWriteCloser(nil), serve.NewConn(nil))
...
func TestHandleUsesDialableAddressForDomainDest(t *testing.T) {
	...
	conn, err := plug.Handle(serve.NewReadWriteCloser(nil), serve.NewConn(nil))
	if err != nil {
...
func TestRelayReturnsBridgeWriteError(t *testing.T) {
	...
	_, err := plug.Relay(serve.NewConn(nil), bridge)
