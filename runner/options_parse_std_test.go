...
func TestOptionsParseArgsShortLongAndDefaults(t *testing.T) {
	...
	if !reflect.DeepEqual(opt.LocalAddr, []string{"port://:8080", "socks5://:1080"}) || !reflect.DeepEqual(opt.RemoteAddr, []string{"port://:9090"}) {
		t.Fatalf("unexpected local/remote: %#v %#v", opt.LocalAddr, opt.RemoteAddr)
	}
	if opt.Alias != "demo" || opt.Redirect != "agent-1" {
		t.Fatalf("unexpected alias/redirect: %s %s", opt.Alias, opt.Redirect)
	}
	...
	if opt.Retry != 0 || opt.RetryInterval != 10 {
...
func TestOptionsParseArgsPositionalError(t *testing.T) {
	var opt Options
	err := opt.ParseArgs([]string{"-l", "socks5://:1080", "unexpected"})
	if err == nil {
...
func TestRemDialCommandLineCompatibility(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			cmdline: "-c 'tcp://nonenonenonenone:@127.0.0.1:34996/?wrapper=raw&compress=1&retry=3&retry-interval=2' -l 'memory+socks5://:@pipe-name' -r 'raw://127.0.0.1:9001'",
		},
	}
...
func TestOptionsParseArgsCombinedShortFlags(t *testing.T) {
	var opt Options
	err := opt.ParseArgs([]string{"-qn", "-ctcp://127.0.0.1:34996"})
	if err != nil {
		t.Fatalf("ParseArgs failed: %v", err)
	}
...
func TestOptionsParseArgsShortAndLongEqualsSyntax(t *testing.T) {
	...
	if opt.Key != "test-key" || opt.IP != "1.2.3.4" {
		t.Fatalf("unexpected short equals parse result: key=%s ip=%s", opt.Key, opt.IP)
	}
...
func TestOptionsPrepareSupportsRelayModeWithServerAndClientTogether(t *testing.T) {
	var opt Options
	err := opt.ParseArgs([]string{
		"-s", "tcp://0.0.0.0:34996",
	...
	if prepared.IsServerMode {
		t.Fatal("expected relay mode to run as client-side console")
	}
	if !prepared.IsRelayMode {
		t.Fatal("expected relay mode when both server and client settings are present")
	}
	if len(prepared.ConsoleURLs) != 1 ||
		prepared.ConsoleURLs[0].Scheme != "tcp" ||
		prepared.ConsoleURLs[0].Hostname() != "127.0.0.1" ||
		prepared.ConsoleURLs[0].Port() != "34996" {
		t.Fatalf("unexpected relay upstream console URLs: %#v", prepared.ConsoleURLs)
	}
	if len(prepared.RelayListenURLs) != 1 ||
		prepared.RelayListenURLs[0].Scheme != "tcp" ||
		prepared.RelayListenURLs[0].Hostname() != "0.0.0.0" ||
		prepared.RelayListenURLs[0].Port() != "34996" {
		t.Fatalf("unexpected relay listen URLs: %#v", prepared.RelayListenURLs)
	}
}
