...
func newRemProxyClient(proxyURL *url.URL, _ proxyclient.Dial) (proxyclient.Dial, error) {
	return func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		console, err := NewConsoleWithCMD(fmt.Sprintf("-c %s -l memory+socks5://:@%s", proxyURL.String(), memoryPipe))
		if err != nil {
			return nil, err
		}
...
func (c *Console) handlerSubscribe() {
	...
	mux.HandleFunc("/topology", func(w http.ResponseWriter, request *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		serverID := "server"
		if c.Config.Alias != "" {
			serverID = c.Config.Alias
		}
		w.Write([]byte(agent.TopologyJSON(serverID)))
	})
