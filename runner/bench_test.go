...
func setupBenchTCP(t *testing.T) string {
	...
	if bin == "" {
		startBenchHelperProcess(t,
			fmt.Sprintf("-c tcp://127.0.0.1:%d/?wrapper=raw -l socks5://127.0.0.1:%d -a %s",
...
func setupBenchUDP(t *testing.T) string {
	...
	if bin == "" {
		startBenchHelperProcess(t,
			fmt.Sprintf("-c udp://127.0.0.1:%d/?wrapper=raw -l socks5://127.0.0.1:%d -a %s",
				serverPort, socksPort, alias))
	}
...
func newBenchSOCKS5Client(t *testing.T, socksAddr string) *http.Client {
	...
	return &http.Client{
		Transport: &http.Transport{
			...
			MaxIdleConns:        256,
			// Bench high-concurrency traffic is meant to stress tunnel request churn,
			// not HTTP connection reuse semantics through SOCKS.
