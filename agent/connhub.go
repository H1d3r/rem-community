...
func (h *ConnHub) Stats() []ConnHubConnStat {
	...
	for _, id := range ids {
		...
		stats = append(stats, ConnHubConnStat{
			...
			RateInBps:  snap.RateInBps,
			RateOutBps: snap.RateOutBps,
		})
	}
	h.mu.RUnlock()
	return stats
...
func (h *ConnHub) AggregateTrafficSnapshot() cio.ConnStatsSnapshot {
	h.mu.RLock()
	defer h.mu.RUnlock()

	var snap cio.ConnStatsSnapshot
	for _, m := range h.conns {
		if m.stats == nil {
			continue
		}
		connSnap := m.stats.Snapshot()
		snap.BytesIn += connSnap.BytesIn
		snap.BytesOut += connSnap.BytesOut
		snap.RateInBps += connSnap.RateInBps
		snap.RateOutBps += connSnap.RateOutBps
		if snap.StartedAt.IsZero() || (!connSnap.StartedAt.IsZero() && connSnap.StartedAt.Before(snap.StartedAt)) {
			snap.StartedAt = connSnap.StartedAt
		}
		if connSnap.LastActive.After(snap.LastActive) {
			snap.LastActive = connSnap.LastActive
		}
	}
	return snap
}
